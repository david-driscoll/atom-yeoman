...
module.exports = function logger() {
    ...
    //
    // Returns the logger
    var log: any = function log(msg, ctx) {
        /*msg = msg || '';
    ...
    Object.keys(colors).forEach(function(status) {
        ...
        log[status] = function() {
            ...
            messages[status].push(_.toArray(<any>arguments).join(' '));
            //this/*.write(chalk[color](pad(status)))*/.write(padding);
            this.write(util.format.apply(util, [status].concat(Array.prototype.slice(<any>arguments))) + '\n');
            return this;
        };
    });
